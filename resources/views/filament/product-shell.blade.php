...
<div class="pm-product-shell" x-data="{ open: false }">
    ...
    <aside class="pm-shell-sidebar" :class="open ? 'is-open' : ''">
        ...
        <nav class="pm-shell-nav" aria-label="Primary navigation">
            ...
            @else
                ...
                @if($user->hasPermissionTo('manage-users', 'web'))<a class="pm-nav-item {{ $active('admin/users') ? 'is-active' : '' }}" href="{{ url('/admin/users') }}"><span class="pm-nav-icon">◎</span><span>People</span></a>@endif
                @if($user->hasPermissionTo('manage-departments', 'web'))<a class="pm-nav-item {{ $active('admin/departments') ? 'is-active' : '' }}" href="{{ url('/admin/departments') }}"><span class="pm-nav-icon">◫</span><span>Departments</span></a>@endif
                @if($user->hasPermissionTo('manage-branches', 'web'))<a class="pm-nav-item {{ $active('admin/branches') ? 'is-active' : '' }}" href="{{ url('/admin/branches') }}"><span class="pm-nav-icon">⌖</span><span>Branches</span></a>@endif
                ...
                @if($user->hasPermissionTo('view-reports', 'web'))<a class="pm-nav-item {{ $active('admin/attendance-reports') ? 'is-active' : '' }}" href="{{ \App\Filament\Pages\AttendanceReports::getUrl() }}"><span class="pm-nav-icon">▥</span><span>Reports</span></a>@endif
                @if($user->hasPermissionTo('manage-settings', 'web'))<p class="pm-nav-label">System</p><a class="pm-nav-item {{ $active('admin/company-settings') ? 'is-active' : '' }}" href="{{ \App\Filament\Pages\CompanySettings::getUrl() }}"><span class="pm-nav-icon">⚙</span><span>Settings</span></a>@endif
            @endif
        </nav>
    ...
    <header class="pm-shell-topbar">
        ...
        <div class="pm-topbar-actions">
            @if (! $isStaff && $user->hasPermissionTo('manage-tasks', 'web'))<a class="pm-topbar-create" href="{{ \App\Filament\Resources\TaskResource::getUrl('create') }}">+ <span>New task</span></a>@endif
