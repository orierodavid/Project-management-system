    ...
    <div class="pm-product-shell" x-data="{ open: false }">
        ...
        <aside class="pm-shell-sidebar" :class="open ? 'is-open' : ''">
            ...
            <nav class="pm-shell-nav" aria-label="Primary navigation">
                @if ($isStaff)
                    ...
                    <a class="pm-nav-item {{ $active('staff') && ! $active('staff/tasks') && ! $active('staff/attendance') ? 'is-active' : '' }}" href="{{ url('/staff') }}"><span class="pm-nav-icon">⌂</span><span>Overview</span></a>
                    ...
                    <a class="pm-nav-item {{ $active('staff/calendar') ? 'is-active' : '' }}" href="{{ url('/staff/calendar') }}"><span class="pm-nav-icon">□</span><span>Calendar</span></a>
                    <a class="pm-nav-item {{ $active('staff/reports') ? 'is-active' : '' }}" href="{{ url('/staff/reports') }}"><span class="pm-nav-icon">▥</span><span>Reports</span></a>
                @else
                    <p class="pm-nav-label">Workspace</p>
                    <a class="pm-nav-item {{ $active('admin') && ! $active('admin/tasks') && ! $active('admin/users') && ! $active('admin/departments') && ! $active('admin/branches') && ! $active('admin/attendance') ? 'is-active' : '' }}" href="{{ url('/admin') }}"><span class="pm-nav-icon">⌂</span><span>Dashboard</span></a>
                    <a class="pm-nav-item {{ $active('admin/tasks') ? 'is-active' : '' }}" href="{{ url('/admin/tasks') }}"><span class="pm-nav-icon">✓</span><span>Tasks</span></a>
                    <a class="pm-nav-item {{ $active('admin/calendar') ? 'is-active' : '' }}" href="{{ url('/admin/calendar') }}"><span class="pm-nav-icon">□</span><span>Calendar</span></a>
            ...
            <div class="pm-shell-account">
                ...
                <div class="pm-account-popover" x-show="open" x-cloak @click.outside="open = false">
                    <form method="POST" action="{{ \Filament\Facades\Filament::getCurrentPanel()->getLogoutUrl() }}">
                        @csrf
        ...
        <header class="pm-shell-topbar">
            ...
            <div class="pm-topbar-actions">
                <button class="pm-topbar-icon" type="button" aria-label="Notifications">♢<i></i></button>
                <div class="pm-topbar-divider"></div>
                <span class="pm-topbar-name">{{ $user->name }}</span>
                <span class="pm-user-avatar pm-user-avatar-small">{{ $initials }}</span>
            </div>
        </header>
    </div>
@endif
